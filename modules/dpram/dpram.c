...
void check_int_pin_level(void)
{
	...
	while (cnt++ < 3) {
		...
		if (gpio_get_value(GPIO_INT_ONEDRAM_AP_N))
			break;
	}
}
...
static void init_hw_setting(void)
{
	...
	s3c_gpio_cfgpin(GPIO_INT_ONEDRAM_AP_N, S3C_GPIO_SFN(GPIO_INT_ONEDRAM_AP_N_AF));
	s3c_gpio_setpull(GPIO_INT_ONEDRAM_AP_N, S3C_GPIO_PULL_UP); 
...
static void check_miss_interrupt(void)
{
	...
	if (gpio_get_value(GPIO_PHONE_ACTIVE) &&
			(!gpio_get_value(GPIO_INT_ONEDRAM_AP_N))) {
