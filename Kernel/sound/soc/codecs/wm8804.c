...
static const u8 wm8804_reg_defs[WM8804_MAX_REGISTER + 1] = {
	0x05,     /* R0  - RST/DEVID1 */
	0x88,     /* R1  - DEVID2 */
	0x04,     /* R2  - DEVREV */
	0x21,     /* R3  - PLL1 */
...
struct wm8804_priv {
	...
	u8 reg_cache[WM8804_MAX_REGISTER + 1];
...
#ifdef CONFIG_PM
static int wm8804_suspend(struct platform_device *pdev, pm_message_t state)
{
...
static int wm8804_resume(struct platform_device *pdev)
{
	struct snd_soc_device *socdev;
	struct snd_soc_codec *codec;
...
static int wm8804_remove(struct platform_device *pdev)
{
	struct snd_soc_device *socdev;
...
static int wm8804_probe(struct platform_device *pdev)
{
	struct snd_soc_device *socdev;
	struct snd_soc_codec *codec;
	int ret;
	...
	socdev = platform_get_drvdata(pdev);
	socdev->card->codec = wm8804_codec;
	codec = wm8804_codec;
...
EXPORT_SYMBOL_GPL(wm8804_dai);

static int wm8804_unregister(struct wm8804_priv *wm8804)
{
	int i;

	wm8804_set_bias_level(&wm8804->codec, SND_SOC_BIAS_OFF);
	for (i = 0; i < ARRAY_SIZE(wm8804->supplies); i++)
		regulator_unregister_notifier(wm8804->supplies[i].consumer,
					      &wm8804->disable_nb[i]);
	regulator_bulk_free(ARRAY_SIZE(wm8804->supplies), wm8804->supplies);
	snd_soc_unregister_dai(&wm8804_dai);
	snd_soc_unregister_codec(&wm8804->codec);
	kfree(wm8804);
	wm8804_codec = NULL;
	return 0;
}

static int wm8804_register(struct wm8804_priv *wm8804,
			   enum snd_soc_control_type control)
{
	...
	if (wm8804_codec) {
		dev_err(codec->dev, "Another WM8804 is registered\n");
		ret = -EINVAL;
		goto err;
	}

	snd_soc_codec_set_drvdata(codec, wm8804);
	codec->name = "wm8804";
	codec->owner = THIS_MODULE;
	codec->bias_level = SND_SOC_BIAS_OFF;
	codec->set_bias_level = wm8804_set_bias_level;
	codec->dai = &wm8804_dai;
	codec->num_dai = 1;
	codec->reg_cache_size = ARRAY_SIZE(wm8804_reg_defs);
	codec->reg_cache = wm8804->reg_cache;
	codec->volatile_register = wm8804_volatile;
	memcpy(codec->reg_cache, wm8804_reg_defs, sizeof(wm8804->reg_cache));
	...
	ret = snd_soc_register_dai(&wm8804_dai);
	if (ret) {
...
struct snd_soc_codec_device soc_codec_dev_wm8804 = {
	.probe = wm8804_probe,
	.remove = wm8804_remove,
	.suspend = wm8804_suspend,
	.resume = wm8804_resume
};
EXPORT_SYMBOL_GPL(soc_codec_dev_wm8804);
...
static int __devinit wm8804_spi_probe(struct spi_device *spi)
{
	...
	dev_set_drvdata(&spi->dev, wm8804);

	codec->control_data = spi;
	codec->dev = &spi->dev;

	return wm8804_register(wm8804, SND_SOC_SPI);
}
...
static int __devexit wm8804_spi_remove(struct spi_device *spi)
{
	...
	wm8804 = dev_get_drvdata(&spi->dev);
	wm8804_unregister(wm8804);
	return 0;
}
				      ...
				      const struct i2c_device_id *id)
{
	...
	return wm8804_register(wm8804, SND_SOC_I2C);
}
...
static __devexit int wm8804_i2c_remove(struct i2c_client *client)
{
	...
	wm8804 = i2c_get_clientdata(client);
	wm8804_unregister(wm8804);
	return 0;
}
