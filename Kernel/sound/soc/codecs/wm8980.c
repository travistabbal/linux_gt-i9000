...
static struct snd_soc_codec *wm8980_codec;

struct wm8980_priv {
	struct snd_soc_codec codec;
	u16 reg_cache[WM8980_CACHEREGNUM];
};
...
static int wm8980_probe(struct platform_device *pdev)
{
	...
	if (wm8980_codec == NULL) {
		dev_err(&pdev->dev, "Codec device not registered\n");
		return -ENODEV;
	}

	socdev->card->codec = wm8980_codec;
	codec = wm8980_codec;
...
static int wm8980_register(struct wm8980_priv *wm8980)
{
	int ret;
	struct snd_soc_codec *codec = &wm8980->codec;

	if (wm8980_codec) {
		dev_err(codec->dev, "Another WM8980 is registered\n");
		return -EINVAL;
	}
	...
	codec->num_dai = 1;
	codec->reg_cache_size = WM8980_CACHEREGNUM;
	codec->reg_cache = &wm8980->reg_cache;

	memcpy(codec->reg_cache, wm8980_reg, sizeof(wm8980_reg));
	...
	ret = snd_soc_register_dai(&wm8980_dai);
	if (ret != 0) {
...
static void wm8980_unregister(struct wm8980_priv *wm8980)
{
	wm8980_set_bias_level(&wm8980->codec, SND_SOC_BIAS_OFF);
	snd_soc_unregister_dai(&wm8980_dai);
	snd_soc_unregister_codec(&wm8980->codec);
	kfree(wm8980);
	wm8980_codec = NULL;
}

#if defined(CONFIG_SPI_MASTER)
static int wm8980_spi_write(struct spi_device *spi, const char *data, int len)
{
...
static int __devinit wm8980_spi_probe(struct spi_device *spi)
{
	...
	wm8980 = kzalloc(sizeof(struct wm8980_priv), GFP_KERNEL);
	if (wm8980 == NULL)
		return -ENOMEM;

	codec = &wm8980->codec;
	codec->control_data = spi;
	codec->hw_write = (hw_write_t)wm8980_spi_write;
	codec->dev = &spi->dev;

	dev_set_drvdata(&spi->dev, wm8980);

	return wm8980_register(wm8980);
}

static int __devexit wm8980_spi_remove(struct spi_device *spi)
{
	struct wm8980_priv *wm8980 = dev_get_drvdata(&spi->dev);

	wm8980_unregister(wm8980);
...
{
	struct wm8980_priv *wm8980 = i2c_get_clientdata(client);
	wm8980_unregister(wm8980);
	return 0;
}
