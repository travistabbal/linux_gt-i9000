...
struct wm8770_priv {
	...
	u16 reg_cache[WM8770_CACHEREGNUM];
...
#ifdef CONFIG_PM
static int wm8770_suspend(struct platform_device *pdev, pm_message_t state)
{
...
static int wm8770_resume(struct platform_device *pdev)
{
	struct snd_soc_device *socdev;
	struct snd_soc_codec *codec;
...
static struct snd_soc_codec *wm8770_codec;

static int wm8770_probe(struct platform_device *pdev)
{
	struct snd_soc_device *socdev;
	struct snd_soc_codec *codec;
	int ret;
	...
	socdev = platform_get_drvdata(pdev);
	socdev->card->codec = wm8770_codec;
	codec = wm8770_codec;
	...
	snd_soc_dapm_new_controls(codec, wm8770_dapm_widgets,
				  ARRAY_SIZE(wm8770_dapm_widgets));
	snd_soc_dapm_add_routes(codec, wm8770_intercon,
				ARRAY_SIZE(wm8770_intercon));
	return 0;
}

static int wm8770_remove(struct platform_device *pdev)
{
	struct snd_soc_device *socdev;
...
EXPORT_SYMBOL_GPL(wm8770_dai);

static int wm8770_register(struct wm8770_priv *wm8770,
			   enum snd_soc_control_type control)
{
	...
	if (wm8770_codec) {
		dev_err(codec->dev, "Another WM8770 is registered\n");
		ret = -EINVAL;
		goto err;
	}
	...
	snd_soc_codec_set_drvdata(codec, wm8770);
	codec->idle_bias_off = 1;
	codec->name = "wm8770";
	codec->owner = THIS_MODULE;
	codec->bias_level = SND_SOC_BIAS_OFF;
	codec->set_bias_level = wm8770_set_bias_level;
	codec->dai = &wm8770_dai;
	codec->num_dai = 1;
	codec->reg_cache_size = ARRAY_SIZE(wm8770_reg_defs);
	codec->reg_cache = wm8770->reg_cache;

	memcpy(codec->reg_cache, wm8770_reg_defs, sizeof(wm8770->reg_cache));
	...
	ret = snd_soc_register_dai(&wm8770_dai);
	if (ret) {
...
static int wm8770_unregister(struct wm8770_priv *wm8770)
{
	int i;

	wm8770_set_bias_level(&wm8770->codec, SND_SOC_BIAS_OFF);
	for (i = 0; i < ARRAY_SIZE(wm8770->supplies); ++i)
		regulator_unregister_notifier(wm8770->supplies[i].consumer,
					      &wm8770->disable_nb[i]);
	regulator_bulk_free(ARRAY_SIZE(wm8770->supplies), wm8770->supplies);
	snd_soc_unregister_dai(&wm8770_dai);
	snd_soc_unregister_codec(&wm8770->codec);
	kfree(wm8770);
	wm8770_codec = NULL;
	return 0;
}

struct snd_soc_codec_device soc_codec_dev_wm8770 = {
	.probe = wm8770_probe,
	.remove = wm8770_remove,
	.suspend = wm8770_suspend,
	.resume = wm8770_resume
};
EXPORT_SYMBOL_GPL(soc_codec_dev_wm8770);
...
static int __devinit wm8770_spi_probe(struct spi_device *spi)
{
	...
	wm8770 = kzalloc(sizeof *wm8770, GFP_KERNEL);
	if (!wm8770)
		return -ENOMEM;

	codec = &wm8770->codec;

	dev_set_drvdata(&spi->dev, wm8770);

	codec->control_data = spi;
	codec->dev = &spi->dev;

	return wm8770_register(wm8770, SND_SOC_SPI);
}
...
static int __devexit wm8770_spi_remove(struct spi_device *spi)
{
	...
	wm8770 = dev_get_drvdata(&spi->dev);
	wm8770_unregister(wm8770);
	return 0;
}
