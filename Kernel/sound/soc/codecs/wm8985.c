...
static const u16 wm8985_reg_defs[WM8985_REGCACHENUM] = {
...
static struct snd_soc_codec *wm8985_codec;
...
static int wm8985_probe(struct platform_device *pdev)
{
	...
	socdev = platform_get_drvdata(pdev);
	socdev->card->codec = wm8985_codec;
	codec = wm8985_codec;
...
static int wm8985_unregister(struct wm8985_priv *wm8985)
{
	wm8985_set_bias_level(&wm8985->codec, SND_SOC_BIAS_OFF);
	snd_soc_unregister_dai(&wm8985_dai);
	snd_soc_unregister_codec(&wm8985->codec);
...
static int wm8985_register(struct wm8985_priv *wm8985,
			   enum snd_soc_control_type control)
{
	...
	if (wm8985_codec) {
		dev_err(codec->dev, "Another WM8985 is registered\n");
		ret = -EINVAL;
		goto err;
	}

	snd_soc_codec_set_drvdata(codec, wm8985);
	...
	codec->num_dai = 1;
	codec->reg_cache_size = ARRAY_SIZE(wm8985->reg_cache);
	codec->reg_cache = wm8985->reg_cache;
	memcpy(codec->reg_cache, wm8985_reg_defs, sizeof(wm8985->reg_cache));
	...
	ret = snd_soc_register_dai(&wm8985_dai);
	if (ret) {
...
static int __devinit wm8985_spi_probe(struct spi_device *spi)
{
	...
	dev_set_drvdata(&spi->dev, wm8985);

	return wm8985_register(wm8985, SND_SOC_SPI);
}
...
static int __devexit wm8985_spi_remove(struct spi_device *spi)
{
	...
	wm8985 = dev_get_drvdata(&spi->dev);
	wm8985_unregister(wm8985);
	return 0;
}
				      ...
				      const struct i2c_device_id *id)
{
	...
	return wm8985_register(wm8985, SND_SOC_I2C);
}
...
static __devexit int wm8985_i2c_remove(struct i2c_client *client)
{
	...
	wm8985 = i2c_get_clientdata(client);
	wm8985_unregister(wm8985);
	return 0;
}
