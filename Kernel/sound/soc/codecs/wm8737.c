...
struct wm8737_priv {
	...
	unsigned int mclk;
	u16 reg_cache[WM8737_REGISTER_COUNT - 1];
};
				 ...
				 enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_STANDBY:
		if (codec->bias_level == SND_SOC_BIAS_OFF) {
			...
			for (i = 0; i < ARRAY_SIZE(wm8737->reg_cache); i++)
				snd_soc_write(codec, i, wm8737->reg_cache[i]);
			   ...
			   enum snd_soc_control_type control)
{
	...
	snd_soc_codec_set_drvdata(codec, wm8737);
	codec->name = "WM8737";
	...
	codec->num_dai = 1;
	codec->reg_cache_size = ARRAY_SIZE(wm8737->reg_cache);
